...
__doc__ = f"""
gf2p8multool {__version__}
  A tool to do some gf2p8mul analysis:
  - guess the key length (based on count of equal chars)
  - guess the key (base on knowledge of most frequent char)

Usage:
  gf2p8multool [-x] [-m MAX-LEN] [-f] [-t CHARSET] [FILE]
  gf2p8multool [-x] [-l LEN] [-c CHAR | -b | -o] [-f] [-t CHARSET] [-p PLAIN] [FILE]
  gf2p8multool [-x] [-m MAX-LEN| -l LEN] [-c CHAR | -b | -o] [-f] [-t CHARSET] [-p PLAIN] [FILE]
  gf2p8multool [-h | --help]
  gf2p8multool --version
...
Examples:
  gf2p8multool file.bin
  gf2p8multool -l 11 -c 20 file.bin
  gf2p8multool -x -c ' ' file.hex
  gf2p8multool -b -f -l 23 -t base64 message.enc
"""
...
from charset import CharsetError
from colors import (
    COLORS,
    C_BEST_KEYLEN,
    C_BEST_PROB,
...
DIRNAME = 'gf2p8multool_out'  # here plaintexts will be placed
PARAMETERS = dict()
    ...
    for offset in range(key_length):  # each byte of key<
        ...
        for char in chars_count:
            if chars_count[char] >= max_count:
                key_possible_bytes[offset].append(gf2p8div(char, most_char))
...
def produce_plaintexts(ciphertext, keys, key_char_used):
    ...
    for index, key in enumerate(keys):
        ...
        degf2p8muled = degf2p8mul(ciphertext, key)
        # ignore saving file when known plain is provided and output doesn't contain it
        if PARAMETERS["known_plain"] and PARAMETERS["known_plain"] not in degf2p8muled:
            continue
        perc = round(100 * percentage_valid(degf2p8muled))
