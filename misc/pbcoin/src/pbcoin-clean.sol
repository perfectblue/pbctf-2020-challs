...
/*
    function customEnc(bytes feed) view public returns(bytes) {
        
        require(seed_in.length == 4);               // Implicitly tells seed length is 4
        
       // require(this.hmacsha256(bytes(seed), "pbctf{not_the_flag}") == 0x660250a58689ff6ec3acb5efeb01d0ea67599efd2482c1761d5fb81c8b7b5976);  // Hash is given out to brute
     
        if(feed.length > 6){                            // Players will see that real flag should be > 6 obviously
            uint offset = feed.length - 2;                  // Total length - 2
            feed[offset] = seed_in[2] ^ feed[offset-8] ^ seed_in[3];
            feed[offset-4] = seed_in[0] ^ feed[offset-2] ^ seed_in[1];
            return feed;
        }
        else{
            assembly{
